...
from cms.carousels.forms import CarouselItemLinkForm
...
class CarouselItemLinkView(UniCMSCachedRetrieveUpdateDestroyAPIView):
    """
    """
    description = ""
    ...
    def get_object(self):
        ...
        return get_object_or_404(CarouselItemLink.objects.select_related('carousel_item'),
                                 pk=item_id,
...
class CarouselItemLinksFormView(APIView):

    def get(self, *args, **kwargs):
        form = CarouselItemLinkForm(carousel_item_id=kwargs.get('carousel_item_id'),
                                    carousel_id=kwargs.get('carousel_id'))
        form_fields = UniCMSFormSerializer.serialize(form)
        return Response(form_fields)


class CarouselItemLinkLogsSchema(AutoSchema):
    def get_operation_id(self, path, method):# pragma: no cover
        return 'listCarouselItemLinkLogs'


class CarouselItemLinkLogsView(ObjectLogEntriesList):

    schema = CarouselItemLinkLogsSchema()
    ...
    def get_queryset(self, **kwargs):
        ...
        item = get_object_or_404(CarouselItemLink.objects.select_related('carousel_item'),
                                 pk=object_id,
                                 carousel_item__carousel__pk=carousel_id,
                                 carousel_item__pk=carousel_item_id)
