...
class UserCanAddPublicationOrAdminReadonly(BasePermission):
    """
    """
    ...
    def has_permission(self, request, view):
        ...
        if not user.is_staff: return False
        if request.method in SAFE_METHODS: return True
        return user.has_perm('cmspublications.add_publication')
...
class UserCanAddMediaCollectionOrAdminReadonly(BasePermission):
    """
    """
    ...
    def has_permission(self, request, view):
        ...
        if not user.is_staff: return False
        if request.method in SAFE_METHODS: return True
        return user.has_perm('cmsmedias.add_mediacollection')
